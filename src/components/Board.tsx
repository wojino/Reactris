...
function Board() {
  ...
  const renderHoldBoard = useCallback(() => {
    const grid = Array(4).fill(null).map(() => Array(2).fill('0'));
    if (!holdPiece) {
      return grid;
    }
  ...
  }, [movePiece, rotatePieceCW, rotatePieceCCW, rotatePiece180, lockPiece, isColliding, piece, board, position, calculateDropDistance, resetPiece, holdCurrentPiece]);
        ...
        <div className="hold">
          {holdBoard.map((row, y) => (
            <div key={y} className="hold-row">
              {row.map((cell, x) => (
                <div key={x} className={`cell ${cell === '0' ? 'empty' : cell}`} />
              ))}
            </div>
          ))}
        </div>
